...
# Standard reference pressure for air (20 μPa)
P_REF_DEFAULT = 20e-6
...
def compute_spl(
    ...
    Args:
        signal: (..., N_t) pressure time series. Can be batched.
        dt: timestep in seconds
        p_ref: reference pressure (default 20 μPa for air)
        nperseg: samples per segment (determines frequency resolution)
        overlap: fractional overlap between segments (0 to 1)

    Returns:
        frequencies: (N_freq,) frequency array in Hz
        spl: (..., N_freq) SPL in dB re p_ref
    """
...
def compute_oaspl(
    ...
    Args:
        signal: (..., N_t, ...) pressure time series
        p_ref: reference pressure (default 20 μPa for air)
        axis: time axis (default -1)

    Returns:
        oaspl: (...) OASPL in dB re p_ref (time axis removed)
    """
...
def compute_oaspl_band(
    ...
    Returns:
        oaspl: (...) band-limited OASPL in dB re p_ref
    """
    device = signal.device
    *batch_dims, N_t = signal.shape
